<div class="pcoded-content">
	<div class="page-header card">
		<div class="row align-items-end">
			
			<div class="col-lg-12">
				<div class="page-header-breadcrumb">
					<ul class=" breadcrumb breadcrumb-title">
						<li class="breadcrumb-item">
							<a href="<?=MY_URL?>"><i class="feather icon-home"></i></a>
						</li>
						<li class="breadcrumb-item">
							Тестҳо
						</li>
						
						<li class="breadcrumb-item">
							Руйхати фанҳо
						</li>
					</ul>
				</div>
			</div>
		</div>
	</div>
	
	<div class="pcoded-inner-content">
		<div class="main-body">
			<div class="page-wrapper">
				<div class="page-body">
					<div class="row">
						
						<!-- Large modal -->
						<div class="modal fade bs-example-modal-lg" tabindex="-1" role="dialog" aria-hidden="true">
							<div class="modal-dialog modal-lg" style="max-width: 80%;">
								<div class="modal-content">
									<div class="modal-header">
										<h6 class="modal-title" id="myModalLabel"></h6>
										<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span></button>
									</div>
									<div class="modal-body" id="bigmodal">
										
									</div>
								</div>
							</div>
						</div>
						<!-- Large modal -->
						
						<div class="col-xl-12 col-md-12">
							<div class="card">
								<div class="card-header">
									<h5>Руйхати фанҳо</h5>
								</div>
								<div class="card-block">
									<!--
									<button data-toggle="modal" data-target=".bs-example-modal-lg" class="add btn btn-inverse waves-effect waves-light" type="button" style="float: right">
										<a data-toggle="modal" data-target=".bs-example-modal-lg" class="add davrifaol" href="javascript:">
											<i class="fa fa-plus"></i> Иловакунӣ
										</a>
									</button>
									-->
									<div style="clear:both"></div>
									<hr>
									<?php //print_arr($fanlist);?>
									<div class="table-responsive davrifaol">
										<table class="table fan" style="font-size:14px">
											<thead class="center">
												<tr style="background-color: #263544; color: #fff">
													<th style="width:70px">№</th>
													<th style="width:70px">ID</th>
													<th style="width:700px">Номгӯи фанҳо</th>
													<th>Холи максимали дар ПК</th>
													<?php foreach($questions_type as $v => $k):?>
														<td>
															<?=$k?>
														</td>
													<?php endforeach;?>
													<th>Амалҳо</th>
												</tr>
											</thead>
											<tbody class="center">
												<?php if(!empty($fanlist)):?>
												
													<?php $counter = 0;?>
													<?php foreach($fanlist as $item):?>
														<tr>
															<th scope="row"><?=++$counter?>.</th>
															<th scope="row"><?=$item['id']?></th>
															<td class="left">
																<?=$item['title_tj']?>
															</td>
															
															<td class="edit" data-id-fan="<?=$item['id_fan']?>" data-field="score" 
															data-s-y="<?=$item['s_y']?>" data-h-y="<?=$item['h_y']?>">
																<?php if($item['score']):?>
																	<?=$item['score']?>
																<?php else:?>
																	30
																<?php endif;?>
															</td>
															
															<?php foreach($questions_type as $v => $k):?>
																<td class="edit" data-id-fan="<?=$item['id_fan']?>" data-field="t<?=$v?>" 
																	data-s-y="<?=$item['s_y']?>" data-h-y="<?=$item['h_y']?>">
																	<?=$item["t$v"]?>
																</td>
															<?php endforeach;?>
															
															<td class="elements">
																<!--
																<a class="info" href="javascript" 
																data-toggle="modal" 
																data-target=".bs-example-modal-lg" 
																data-id="<?=$item['id']?>"
																data-fan="<?=$item['title_tj']?>"
																>
																	<i class="fa fa-info"></i>
																</a>
																-->
																
																<a href="<?=MY_URL?>?option=tests&action=downloadtest&id_fan=<?=$item['id_fan']?>">
																	<i class="fa fa-download"></i>
																</a>
																
															</td>
														</tr>
													<?php endforeach;?>
												<?php else: ?>
													<tr class="center bold">
														<td colspan="4">
															<i class="fa fa-warning"></i> Маълумот нест.
														</td>
													</tr>
												<?php endif;?>
											</tbody>
										</table>
									</div>
									
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>



<script type="text/javascript">
	jQuery(document).ready(function($){
		
		$('.fan').on('click', 'td.edit', function(){
			//находим input внутри элемента с классом ajax и вставляем вместо input его значение
			var value = $('.ajax input').val();
			$('.ajax').html(value);
			//удаляем все классы ajax
			$('.ajax').removeClass('ajax');
			//Нажатой ячейке присваиваем класс ajax
			$(this).addClass('ajax');
			//внутри ячейки создаём input и вставляем текст из ячейки в него
			
			var val = $(this).text().trim();
			$(this).html('<input style="width:65px;" autocomplete="off" id="editbox" class="form-control" value="' + val + '" type="text">');
			//устанавливаем фокус на созданном элементе
			$('#editbox').focus().select();
			
		});
		
		
		
		$('.fan').on('keydown', 'td.edit', function(event){
			var id = $(this).attr('data-id');
			var id_fan = $(this).attr('data-id-fan');
			var field = $(this).attr('data-field');
			var s_y = $(this).attr('data-s-y');
			var h_y = $(this).attr('data-h-y');
			var value = $('.ajax input').val();
			
			if(event.which == 13){
				
				var my_url = '<?=MY_URL;?>';
				var url = '<?=URL."modules/tests/tests_ajax.php?option=updateSetting";?>';
				
				$.ajax({
					type: 'post',
					url: url,
					data: {
						"id": id,
						"id_fan": id_fan,
						"field": field,
						"s_y": s_y,
						"h_y": h_y,
						"value": value
					},
					success: function(res){
						$('.ajax').html(value);
					}
				});
			}
			
			
		});
		
		
		
	});
</script>