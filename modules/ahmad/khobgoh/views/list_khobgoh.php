							...
							<div class="card">
								...
								<div class="card-block">
									...
									<div style="text-align: center">
										<a href="<?=MY_URL?>?option=khobgoh&action=list_omuzgor">
											<div class="letters">Ҳама</div>
										</a>
										<?php foreach($alphabet as $item):?>
											<a href="<?=MY_URL?>?option=khobgoh&action=list_omuzgor&letter=<?=$item?>">
											<div class="letters">
												<?=$item?>
											</div>
											</a>
										...
										<tbody class="center" id="tbody">
											...
											<?php foreach($teachers as $item):?>
												...
												<tr>
													...
													<td>
														<?=count_summa_where("rasidho", "check_money", "`id_student` = '{$item['id']}' AND `type` = '5' AND `s_y` = '".S_Y."'");?>
													</td>
													<td>
														
														<a href="<?=MY_URL?>?option=khobgoh&action=delete_user&id=<?=$item['id']?>" onclick="return confirmDel()" class="btn btn-danger waves-effect waves-light">
...
<script type="text/javascript">
	jQuery(document).ready(function($){
		...
		$('.student_shartnoma').on('click', function(){
			...
			var my_url = '<?=MY_URL;?>';
			var url = '<?=URL."modules/{$option}/{$option}_ajax.php?option=getOrderForm";?>';
