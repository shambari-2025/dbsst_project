...
switch($option){
	...
	case "getimtinfo":
		...
		$info_start = query("SELECT `s_y` FROM `students` WHERE `id_student` = '$id_student' ORDER BY `s_y` ASC");
		
		//$start_s_y = $info_start[0]['s_y'];
		$start_s_y = S_Y - $id_course + 1;
		
		
		$info_end = query("SELECT `s_y` FROM `students` WHERE `id_student` = '$id_student' ORDER BY `s_y` DESC");
	...
	case "getimtinfo_SY_HY":
		...
		if($c_semestr == $semestr){
			...
			AND `results`.`id_student` = '$id_student' AND `results`.`semestr` = '$semestr'
			
			INNER JOIN `fan_test` ON `fan_test`.`id` = `iqtibosho`.`id_fan`
			
			WHERE
				`results`.`id_student` = '$id_student' 
				AND `results`.`s_y` = '$s_y' AND `results`.`semestr` = '$semestr'
			ORDER BY `results`.`id_fan`
			");
			
			//print_arr($fanlist);
		}else{
			$fanlist = getResultsBySemestr($id_student, $id_nt, $semestr);
		}
	...
	case "getstudenttrimestrform":
		...
		$info_std=query("SELECT * FROM `students` WHERE `id` IN (SELECT MAX(`id`) FROM `students` WHERE `students`.`id_student` = '$id_student' AND `s_y` = '$S_Y' AND `h_y` = '$H_Y')");
		$id_faculty=$info_std[0]['id_faculty'];
		$id_course=$info_std[0]['id_course'];
		$id_spec=$info_std[0]['id_spec'];
