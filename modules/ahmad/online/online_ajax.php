...
switch($option){
	...
	case "getimtinfo":
		...
		$info = select("students", "*", "`id_student` = '$id_student'", "id", false, "1");
		$start_s_y = $info[0]['s_y'];
		
		$STUDY_YEARS = select("study_years", "*", "`id` >= '$start_s_y'", "id", false, "");
		...
		ROUND(SUM(`std_teacherscore`.`h9`+`std_teacherscore`.`h10`+`std_teacherscore`.`h11`+`std_teacherscore`.`h12`+`std_teacherscore`.`h13`+`std_teacherscore`.`h14`+`std_teacherscore`.`h15`+`std_teacherscore`.`h16`+`std_teacherscore`.`rating_2`) / 2, 2) as `r_2`,
		...
		LEFT JOIN  `dop_results` ON `nt_content`.`id_fan` = `dop_results`.`id_fan`
