<form action="<?=MY_URL?>?option=soxtor&action=dep_member_update" method="post" enctype="multipart/form-data">
	<table class="addform">
		<tr>
			<td>
				...
				<select name="id_teacher" id="id_teacher" required class="form-control">
					...
					<?php foreach($all_teachers as $item):?>
						<option <?php if($info[0]['id_teacher'] == $item['id']):?>selected<?php endif;?> value="<?=$item['id']?>"><?=$item['fullname_tj']?></option>
					<?php endforeach;?>
				</select>
			</td>
		</tr>
			...
			<td>
				...
				<select name="id_credits_table" id="id_credits_table" required class="form-control">
					...
					<?php foreach($vazifaho as $item):?>
						<option <?php if($info[0]['id_credits_table'] == $item['id']):?>selected<?php endif;?> value="<?=$item['id']?>"><?=$item['vazifa']?></option>
					<?php endforeach;?>
				</select>
			...
			<td>
				...
				<select name="id_worker_type" id="id_worker_type" required class="form-control">
					...
					<?php foreach($worker_type as $item => $value):?>
						<option <?php if($info[0]['id_worker_type'] == $item):?>selected<?php endif;?> value="<?=$item?>"><?=$value?></option>
					<?php endforeach;?>
				</select>
			</td>
		</tr>
