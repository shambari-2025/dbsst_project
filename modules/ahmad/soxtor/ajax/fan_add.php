<form action="<?=MY_URL?>?option=soxtor&action=fan_insert" method="post" enctype="multipart/form-data">
	<table class="addform" >
		<tr>
			<td>
			...
			<td>
				...
				<select name="id_departament" id="id_departament" class="form-control">
					...
					<?php foreach($departaments as $item):?>
						<option <?php if($id_departament == $item['id']):?> selected <?php endif;?> value="<?=$item['id']?>"><?=$item['title']?></option>
					<?php endforeach;?>
				</select>
			</td>
