						...
						<div class="col-sm-12">
							...
							<div class="card">
								<div class="card-header">
									<h5>Донишҷуён: <i><?=getDistrict($id_district)?></i></h5>
								</div>
									...
									<table class="table" style="font-size: 14px !important">
										...
										<tbody class="center">
											...
											<?php foreach($students as $item):?>
												<?php $group_info = query("SELECT * FROM `std_study_groups` WHERE `id_faculty` = '".$item['id_faculty']."' AND `id_s_v` = '".$item['id_s_v']."' AND `id_course` = '".$item['id_course']."' AND `id_spec` = '".$item['id_spec']."' AND `s_y` = '".S_Y."' AND `h_y` = '".H_Y."'");?>
												<tr>
													...
													<td>
														...
														<div class="dropdown-inverse dropdown open">
															...
															<div class="dropdown-menu" aria-labelledby="dropdown-7" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut">
																...
																<a data-toggle="modal" data-target=".bs-example-modal-lg" data-id-student="<?=$item['id']?>" data-name="<?=$item['fullname']?>" data-id-nt="<?=$group_info[0]['id_nt']?>" data-id-course="<?=$group_info[0]['id_course']?>" data-id-s-v="<?=$group_info[0]['id_s_v']?>"
																	class="dropdown-item waves-light waves-effect sessions_results" href="javascript:">
																	<i class="fa fa-line-chart"></i> Натиҷаи сессияҳо
																</a>
																
																<a target="_blank" class="dropdown-item waves-light waves-effect" href="<?=MY_URL?>?option=print&action=transcript&id_student=<?=$item['id']?>"><i class="fa fa-print"></i> Чопи транскрипсия</a>
																<!--
																...
																<a class="dropdown-item waves-light waves-effect" href="<?=MY_URL?>?option=students&action=list&id_faculty=<?=$item['id_faculty']?>&id_s_v=<?=$item['id_s_v']?>&id_course=<?=$item['id_course']?>&id_spec=<?=$item['id_spec']?>&id_group=<?=$item['id_group']?>"><i class="fa fa-mortar-board"></i> Гузариш ба гурӯҳ</a>
