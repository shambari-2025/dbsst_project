	...
	<table class="table" style="font-size:14px">
		<thead class="center">
			<tr style="background-color: #263544; color: #fff">
				...
				<th rowspan="2" style="width: 30px"><div class="vertical">ID FAN</div></th>
				<th rowspan="2" class="left" style="width:350px !important">Номӯйи фанҳо</th>
				<th rowspan="2" >Омӯзгор(он)</th>
				<th rowspan="2">Намуди<br>санҷиш</th>
				<th rowspan="2" >Санаи<br>супоридани<br>имтиҳон</th>
				<th rowspan="2"><div class="vertical">Кредит</div></th>
				
				
				<th colspan="4">Холи<br>Р1</th>
				<th colspan="4">Холи<br>Р2</th>
				
				<th rowspan="2"><div class="vertical">Имтиҳон</div></th>
				<th rowspan="2"><div class="vertical">Триместр</div></th>
				<th rowspan="2"><div class="vertical">Ҷамъ</div></th>
				<th rowspan="2"><div class="vertical">Бо ҳуруф</div></th>
				<th rowspan="2"><div class="vertical">Бо адад</div></th>
				<th rowspan="2">Амалҳо</th>
			</tr>
			...
			<?php foreach($fanlist as $item):?>
				<tr>
					...
					<?php if(!in_array($item['fan_id'], NOT_RATING)):?>
						...
						data-id-fan="<?=$item['fan_id']?>" field="nf_1_score" data-id="<?=@$item['results_id']?>">
							<?=$item['nf_1_score']?>
						</td>
						
						<td class="edit <?php if($item['nf_1_absent'] < 10){ echo "grey";}?>" data-table="results"
						data-id-fan="<?=$item['fan_id']?>" field="nf_1_absent" data-id="<?=@$item['results_id']?>">
							<?=$item['nf_1_absent']?>
						</td>
						
						<td class="edit <?php if($item['nf_1_score_r'] < 10){ echo "grey";}?>" data-table="results"
						data-id-fan="<?=$item['fan_id']?>" field="nf_1_score_r" data-id="<?=@$item['results_id']?>">
							<?=$item['nf_1_score_r']?>
							
							<?php if(isset($_SESSION['user']['admin'])):?>
								<a target="_blank" href="<?=MY_URL."?option=students&action=results_rating&id_student=$id_student&id_fan={$item['fan_id']}&s_y=$s_y&h_y=$h_y";?>">
						...
						<td class="bold">
							<?=$R1 = $item['nf_1_score']+$item['nf_1_absent']+$item['nf_1_score_r']?>
						</td>
						
						<!-- -->
						
						<td class="edit <?php if($item['nf_2_score'] < 10){ echo "grey";}?>" data-table="results"
						data-id-fan="<?=$item['fan_id']?>" field="nf_2_score" data-id="<?=@$item['results_id']?>">
							<?=$item['nf_2_score']?>
						</td>
						
						<td class="edit <?php if($item['nf_2_absent'] < 10){ echo "grey";}?>" data-table="results"
						data-id-fan="<?=$item['fan_id']?>" field="nf_2_absent" data-id="<?=@$item['results_id']?>">
							<?=$item['nf_2_absent']?>
						</td>
						
						<td class="edit <?php if($item['nf_2_score_r'] < 10){ echo "grey";}?>" data-table="results"
						data-id-fan="<?=$item['fan_id']?>" field="nf_2_score_r" data-id="<?=@$item['results_id']?>">
							<?=$item['nf_2_score_r']?>
							<?php if(isset($_SESSION['user']['admin'])):?>
								<a target="_blank" href="<?=MY_URL."?option=students&action=results_rating&id_student=$id_student&id_fan={$item['fan_id']}&s_y=$s_y&h_y=$h_y";?>">
									<i class="fa fa-line-chart"></i>
						...
						<td class="bold">
							<?=$R2 = $item['nf_2_score']+$item['nf_2_absent']+$item['nf_2_score_r']?>
						</td>
						...
						<td class="edit" data-table="results"
						data-id-fan="<?=@$item['fan_id']?>" field="trimestr_score" data-id="<?=@$item['results_id']?>">
							<?=@$item['trimestr_score']?>
							<?php if(!is_null($item['trimestr_score'])):?>
								<?php $item['total_score'] = $item['trimestr_score'];?>
							<?php endif;?>
						</td>
					...
					<td class="elements">
						...
						<?php if(isset($item['id_iqtibos'])):?>
							<?php if(in_array($_SESSION['user']['id'], [1, 2, 10])):?>
							...
							<?php if(@$query[0]['imt_type'] == 1 && (in_array($_SESSION['user']['id'], [1, 2, 10]))):?>
								<?php if(!is_null($item['imt_score'])):?>
