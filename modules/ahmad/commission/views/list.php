							...
							<div class="card">
								...
								<div class="card-block">
									...
									<?php if(isset($_SESSION['user']['admin']) && empty($students)):?>
										...
										href="<?=MY_URL?>?option=commission&action=del_group&id=<?=$id_study_group?>">Несткунии гуруҳ</a>
									<?php endif;?>
									<br>
									<br>
										...
										<tbody class="center" id="tbody">
											...
											<?php foreach($students as $item):?>
												...
												<tr id="student_<?=$item['id']?>">
													...
													<td>
													<?=getStudentCountry($item['id_student'])?>
													</td>
													<td>
														<?php if($item['id_s_t'] == 1):?>
															<?=getShartnomaNumber($item['id'])?><br>
															аз <?=substr($item['added_time'], 0, -10)?>
														<?php endif;?>
													</td>
													<td>
														...
														<?php if($item['id_s_t'] == 1):?>
															<br>
															<?=$sh_t = getSharnomaMoneyBySY($item['id_course'], $item['id_spec'], $item['id_s_l'], $item['id_s_v'], S_Y, $item['foreign']);?>
														<?php endif;?>
													</td>
													...
													<td>
														<?php if($item['id_s_t'] == 1):?>
															<?php $checks = query("SELECT * FROM `rasidho` WHERE `type` = '2' AND `payed` = '1' AND `id_student` = '".$item['id']."' ORDER BY `id` DESC LIMIT 1");?>
																<?php if(!empty($checks)):?>
																	<span class="text-c-green bold">Пардохтшуда <br> <?=$mablag_suporid = $checks[0]['check_money']?></span>
																<?php else:?>
																	<span class="text-c-red bold">Пардохтношуда</span>
																<?php endif;?>
														<?php endif;?>
													</td>
													
													<td>
														<?php if($item['id_s_t'] == 1 && isset($mablag_suporid)):?>
															<?=$sh_t - $mablag_suporid;?>
														<?php endif;?>
													</td>
														...
														<div class="dropdown-inverse dropdown open">
															...
															<div class="dropdown-menu" aria-labelledby="dropdown-7" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut">
																...
																<a class="dropdown-item waves-light waves-effect" 
																	target="_blank" href="<?=MY_URL?>?option=print&action=print_check&type=1&id=<?=$item['id']?>">
																	<i class="fa fa-print"></i> Расид барои ҳуҷҷатҳо
																</a>
																
																<?php if($item['id_s_t'] == 1):?>
																<a class="dropdown-item waves-light waves-effect" 
																	target="_blank" href="<?=MY_URL?>?option=print&action=print_check&type=2&id=<?=$item['id']?>">
																	<i class="fa fa-print"></i> Расид барои шартнома
																</a>
																<?php endif;?>
																-->
																<a class="dropdown-item waves-light waves-effect" 
																	target="_blank" href="<?=MY_URL?>?option=print&action=dovtalab_info&id=<?=$item['id']?>">
																	<i class="fa fa-print"></i> Делои шахси
																</a>
																
																<a class="dropdown-item waves-light waves-effect" 
																	target="_blank" href="<?=MY_URL?>?option=print&action=ariza&id=<?=$item['id']?>">
																	<i class="fa fa-print"></i> Аризаи довталаб
																</a>
																<a class="dropdown-item waves-light waves-effect" 
																	target="_blank" href="<?=MY_URL?>?option=print&action=zabonkhat&id=<?=$item['id']?>">
																	<i class="fa fa-print"></i> Забонхати довталаб
																</a>
																...
																<?php if(isset($_SESSION['user']['admin'])):?>
																	...
																	<a class="dropdown-item waves-light waves-effect" 
																		href="<?=MY_URL?>?option=commission&action=delete_dovtalab&id=<?=$item['id']?>">
