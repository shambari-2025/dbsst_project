	...
	<body style="width:94%; margin: 15px auto 0 auto">
		...
		<table class="table transcript" style="width:100%; font-size:13px;">
			...
			<tbody>
				...
				<tr class="center">
					<td><?=$i;?>.</td>
					<td class="left"><?=getFanTest($item['id_fan'])?><?php if($item['type'] == 2) {echo "<b> (Кори курсӣ)</b>";}?>[<?=$item['id_fan']?>]</td>
					<td><?php if($item['credit'] != 0) {echo $item['credit'];}else{echo "-";}?></td>
					<td><?=getStudyYear($item['s_y'])?></td>
					<td><?=$item['h_y'];?></td>
					<td><?=$total_score = getTotalScore($id_student, $item['id_fan'], $item['s_y'], $item['h_y'])?></td>
