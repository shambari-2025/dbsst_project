...
<html lang="en">
	...
	<body style="width:94%; margin: 15px auto 0 auto; font-size:15px">
		...
		<h2 class="center">Натиҷаҳои имтиҳон дар соли таҳсили 20<?=S_Y?>-20<?=S_Y+1?>, нимсолаи <?=H_Y?></h2>
				...
				<tbody>
					...
					<?php foreach($iqtibosho as $iqtibos):?>
						...
						<tr>
							...
							<td class="center"><?=$std_suporid=count_table_where("results", "`id_faculty` = '$id_faculty' AND `id_s_l` = '$id_s_l' AND `id_s_v` = '$id_s_v' AND `id_course` = '$id_course' AND `id_spec` = '$id_spec' AND `id_group` = '$id_group' AND `id_fan`= '$id_fan' AND IFNULL(`imt_score`, 0) > 0 AND `s_y` = '".S_Y."' AND `h_y` = '".H_Y."'")?></td>
							...
							<td>
								...
								<?php if($std_nasuporid == $std_count):?>
									<b style="color: red">Аз ин фан ягон донишҷӯ имтиҳон насупоридааст</b>
								<?php endif;?>
							</td>
						</tr>
