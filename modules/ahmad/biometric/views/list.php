														...
														<div class="dropdown-inverse dropdown open">
															...
															<div class="dropdown-menu" aria-labelledby="dropdown-7" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut">
																...
																<a data-toggle="modal" data-target=".bs-example-modal-lg" data-id-student="<?=$item['id']?>" data-name="<?=$item['fullname_tj']?>"
																	class="dropdown-item waves-light waves-effect biometric" href="javascript:">
																	<i class="fa fa-edit"></i> Гирифтани биометрик
																</a>
...
<script type="text/javascript">
	jQuery(document).ready(function($){
		...
		$('.biometric').on('click', function(){
			var id_student = $(this).attr("data-id-student");
			var name = $(this).attr("data-name");
			
			var my_url = '<?=MY_URL;?>';
			var url = '<?=URL."modules/{$option}/{$option}_ajax.php?option=biometric";?>';
			
			$('.modal-dialog').css("max-width", "70%");
			$('.modal-title').text("Гирифтани маълумотҳои биометрикӣ: " + name);
